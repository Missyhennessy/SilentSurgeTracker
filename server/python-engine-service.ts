...
`;
  ...
  // Sentiment Analysis Engine (Premium Feature)
  app.post('/api/python-engine/sentiment-analysis', requireSubscription || ((req: any, res: any, next: any) => next()), async (req, res) => {
    try {
...
try:
    ...
    result = analyzer.analyze_crypto_sentiment("${symbol}", "${timeframe}")
    ...
    result_dict = {
        ...
        'influencer_sentiment': result.influencer_sentiment,
        'volume_weighted_sentiment': result.volume_weighted_sentiment,
...
`;
      ...
      const result = await pythonEngineService.executePythonScript('temp_sentiment.py', [], true, script);
      const parsedResult = typeof result === 'string' ? JSON.parse(result) : result;
  ...
  // Volatility Forecasting Engine (Premium Feature)
  app.post('/api/python-engine/volatility-forecast', requireSubscription || ((req: any, res: any, next: any) => next()), async (req, res) => {
    try {
      const { symbol, price_data, returns_data } = req.body;
...
import os
sys.path.append('python_engine')
from volatility_forecasting import CryptoVolatilityForecaster
import json
...
try:
    ...
    returns_data = ${JSON.stringify(returns_data || null)}
    
    result = forecaster.forecast_volatility("${symbol}", price_data, returns_data)
    
    # Convert dataclass to dict
    result_dict = {
        'symbol': result.symbol,
        'current_volatility': result.current_volatility,
        'predicted_volatility_1d': result.predicted_volatility_1d,
        'predicted_volatility_7d': result.predicted_volatility_7d,
        'predicted_volatility_30d': result.predicted_volatility_30d,
...
`;
      ...
      const result = await pythonEngineService.executePythonScript('temp_volatility.py', [], true, script);
      const parsedResult = typeof result === 'string' ? JSON.parse(result) : result;
      
      res.json(parsedResult);
      
    } catch (error: any) {
      console.error('Volatility Forecasting error:', error);
      res.status(500).json({ 
        error: 'Volatility forecasting failed',
        details: error.message 
      });
    }
  });

  // Arbitrage Detection Engine (Premium Feature)
  app.post('/api/python-engine/arbitrage-detection', requireSubscription || ((req: any, res: any, next: any) => next()), async (req, res) => {
    try {
      const { symbol, investment_amount = 10000 } = req.body;
...
import os
sys.path.append('python_engine')
from arbitrage_detection import CryptoArbitrageDetector
import json

try:
    detector = CryptoArbitrageDetector()
    opportunities = detector.detect_arbitrage_opportunities("${symbol}", ${investment_amount})
...
`;
      ...
      const result = await pythonEngineService.executePythonScript('temp_arbitrage.py', [], true, script);
      const parsedResult = typeof result === 'string' ? JSON.parse(result) : result;
  ...
  // Market Analysis Suite - Combines all three engines (Premium Feature)
  app.post('/api/python-engine/market-analysis-suite', requireSubscription || ((req: any, res: any, next: any) => next()), async (req, res) => {
    try {
      const { symbol, investment_amount = 10000, timeframe = '24h' } = req.body;
...
try:
    ...
    volatility_forecaster = CryptoVolatilityForecaster()
    arbitrage_detector = CryptoArbitrageDetector()
    
    # Run sentiment analysis
    sentiment_result = sentiment_analyzer.analyze_crypto_sentiment("${symbol}", "${timeframe}")
    
    # Run volatility forecasting
    volatility_result = volatility_forecaster.forecast_volatility("${symbol}")
    
    # Run arbitrage detection
    arbitrage_opportunities = arbitrage_detector.detect_arbitrage_opportunities("${symbol}", ${investment_amount})
    ...
    sentiment_dict = {
        ...
        'trending_topics': sentiment_result.trending_topics[:3]
    }
    
    volatility_dict = {
        'current_volatility': volatility_result.current_volatility,
        'predicted_volatility_1d': volatility_result.predicted_volatility_1d,
        'predicted_volatility_7d': volatility_result.predicted_volatility_7d,
        'volatility_regime': volatility_result.volatility_regime,
        'volatility_trend': volatility_result.volatility_trend,
        'var_95': volatility_result.var_95
    }
    
    arbitrage_dict = {
        'opportunities_found': len(arbitrage_opportunities),
        'best_opportunity': arbitrage_opportunities[0].__dict__ if arbitrage_opportunities else None,
        'total_potential_profit': sum(opp.estimated_profit for opp in arbitrage_opportunities[:3])
    }
    ...
    analysis_result = {
        ...
        'volatility_forecast': volatility_dict,
        'arbitrage_analysis': arbitrage_dict,
        'overall_recommendation': 'BUY' if sentiment_result.overall_sentiment > 0.2 and volatility_result.volatility_regime in ['low', 'medium'] else 'HOLD' if sentiment_result.overall_sentiment > -0.2 else 'SELL',
        'risk_score': round((volatility_result.var_95 + (100 - sentiment_result.fear_greed_index)) / 20, 1),
        'confidence_score': round((sentiment_result.confidence + volatility_result.confidence) / 2, 3)
    }
...
`;
      ...
      const result = await pythonEngineService.executePythonScript('temp_market_suite.py', [], true, script);
      const parsedResult = typeof result === 'string' ? JSON.parse(result) : result;
      
      res.json(parsedResult);
      
    } catch (error: any) {
      console.error('Market Analysis Suite error:', error);
      res.status(500).json({ 
        error: 'Market analysis suite failed',
        details: error.message 
      });
    }
  });
