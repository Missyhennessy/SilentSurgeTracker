...
export default function SidebarDebug({ activeModule, onModuleChange, isConnected }: SidebarProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  
  const handleModuleClick = (moduleId: DashboardModule, moduleType: string) => {
    console.log(`${moduleType} module clicked:`, moduleId);
    console.log('Current active module:', activeModule);
    console.log('onModuleChange function:', typeof onModuleChange);
    
    try {
      onModuleChange(moduleId);
      console.log('Module change successful for:', moduleId);
    } catch (error) {
        ...
        {modules.map((module) => {
          ...
          return (
            <Button
              ...
              onClick={() => handleModuleClick(module.id, 'Analysis')}
            >
        ...
        {analyticsModules.map((module) => {
          ...
          return (
            <Button
              ...
              onClick={() => handleModuleClick(module.id, 'Analytics')}
            >
              <IconComponent className="w-4 h-4" />
              <span>{module.name}</span>
            </Button>
        ...
        {advancedModules.map((module) => {
          ...
          return (
            <Button
              ...
              onClick={() => handleModuleClick(module.id, 'Advanced')}
            >
              <IconComponent className="w-4 h-4" />
              <span>{module.name}</span>
            </Button>
